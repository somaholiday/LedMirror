/*
 ...
 from Creative Code Workshop on 18 March 2017
 
 Amie Baumgartner
 
 */

class Scene_AmieBaumgartner extends Scene {
  ...
  void _setup() {
    ...
    circles.add(new Circle(new PVector(random(width/2-num, width/2+num), random(height/2-num, height/2+num)), (int)random(min, max)));
  }
  ...
  void _draw() {
    ...
    if (circles.size() < 500) {
      circles.add(new Circle(newLoc, newD));
    }
